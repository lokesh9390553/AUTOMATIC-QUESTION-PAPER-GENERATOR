...
if((name.length==0) || (address.length==0) || (qualification.length==0) || (phone.length==0) || (mobile.length==0) || (email.length==0)   ) 
{
...
<form name=staff action=staffups.jsp  onSubmit='return formvalid()'>
<center>
...
<input value=<%=rs.getString(2)%> name=name type=text  size=30  ></td></tr>
<tr><td class=td><b>Address</b></td><td>
<input value=<%=rs.getString(3)%>  name=address type=text  size=30  ></td></tr>
<tr><td class=td><b>Qualification</b></td><td>
<input value=<%=rs.getString(4)%> name=qualification type=text  size=30  ></td></tr>
<tr><td class=td><b>Phone Num</b></td><td>
<input value=<%=rs.getInt(5)%> name=phone type=text  size=10 onkeypress='if(event.keyCode<48 || event.keyCode>58){staff.phone.focus(); return false;}if(staff.phone.value.length>10)return false;'></td></tr>
<tr><td class=td><b>Mobile Num</b></td><td>
<input name=mobile value=<%=rs.getInt(6)%> type=text  size=10 onkeypress='if(event.keyCode<48 || event.keyCode>58){staff.mobile.focus(); return false;}if(staff.mobile.value.length>10)return false;'></td></tr>
<tr><td class=td><b>Email Id</b></td><td>
<input value=<%=rs.getString(7)%>  name=email type=text  size=20  ></td></tr>
...
<Select name=sex  >
<option value=m selected><%=rs.getString(9)%></option>
...
String msgdate=fmt.format(rs.getDate(11)).toString();
%>

<tr><td class=td><b>Date of Registrtion</b></td><td>
<input name=sel_date type=text  value=<%=msgdate%>   size=10>
